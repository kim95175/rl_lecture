...
class DQNAgent(object):
    def __init__(self, env, agent_params):
        ...
        self.exploration_model = None
        
        # Use this when you want to use RND
    ...
    def train(self, obs, action, reward, next_obs, done):
        ...
        if (self.t > self.learning_starts
            ...
            error = None 
            expl_bonus = None #normalize(error, error.mean(), error.std())
            ...
            if explore_weight > 0:
                ...
                mixed_reward = reward * exploit_weight

            # TODO 1): Update the exploration model (based off s')
            expl_model_loss = None

            # Update the DQN (based off mixed_reward)
            log = self.dqn.update(
